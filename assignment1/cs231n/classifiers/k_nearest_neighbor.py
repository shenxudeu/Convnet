...
class KNearestNeighbor:
  ...
  def compute_distances_one_loop(self, X):
    ...
    dists = np.zeros((num_test, num_train))
    for i in xrange(num_test):
        dists[i,:] = np.sqrt(np.sum(np.square(np.tile(X[i,:],(self.X_train.shape[0],1)) - self.X_train),axis=1))
    return dists
  ...
  def compute_distances_no_loops(self, X):
    ...
    num_train = self.X_train.shape[0]
    dists = np.zeros((num_test, num_train)) 
    z1 = np.tile(np.dot(X,X.T).diagonal().reshape((num_test,1)),(1,num_train))
    z2 = 2.* np.dot(X,self.X_train.T)
    z3 = np.tile(np.dot(self.X_train,self.X_train.T).diagonal().reshape((1,num_train)),(num_test,1))
    dists = np.sqrt(z1-z2+z3)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      # the ith test point.
      closest_y = []
      sorted_ids = np.argsort(dists[i,:])
      closest_y = self.y_train[sorted_ids[:k]]
      max_count = -1
      for label in sorted(list(set(closest_y))):
          count_one = len(closest_y[closest_y==label])
